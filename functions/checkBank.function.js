...
//khong lam duoc do ICB khong dump
    ...
    for (const screen of screenKeywords) {
      if (
        ...
        await sendTelegramAlert(
          telegramToken,
          chatId,
          `Cảnh báo! Phát hiện thao tác thủ công SHB SAHA ở màn hình: ${screen.name} (id thiết bị: ${device_id})`
        );

        Logger.log(1, `Cảnh báo! Phát hiện thao tác thủ công SHB SAHA ở màn hình: ${screen.name} (id thiết bị: ${device_id})`, __filename);

        await saveAlertToDatabase({
          timestamp: new Date().toISOString(),
          reason: `Phát hiện thao tác thủ công SHB SAHA ở màn hình: ${screen.name} (id thiết bị: ${device_id})`,
          filePath: localPath
        });
    ...
    for (const screen of screenKeywords) {
      if (
        ...
        await sendTelegramAlert(
          telegramToken,
          chatId,
          `Cảnh báo! Phát hiện thao tác thủ công khi xuất với MB ở màn hình: ${screen.name} (id thiết bị: ${device_id})`
        );

        Logger.log(1, `Cảnh báo! Phát hiện thao tác thủ công khi xuất với MB ở màn hình: ${screen.name} (id thiết bị: ${device_id})`, __filename);

        await saveAlertToDatabase({
          timestamp: new Date().toISOString(),
          reason: `Phát hiện thao tác thủ công khi xuất với MB ở màn hình: ${screen.name} (id thiết bị: ${device_id})`,
          filePath: localPath
        });
        return;
      }
