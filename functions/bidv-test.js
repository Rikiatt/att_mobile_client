...
const start = async (routing) => {
  try {
    let bankinfo = await request_handler(
      { pin: '' },
      "patch",
      endpoint + `/public/bank_info?auth=` + routing.port,
      response => response.data.result,
      [{ header: { "merchant-id": merchant_id, signature } }])
    let { bankuser: username, bankpass: password, bank_account: account_number, extra: extend, site: page, state } = bankinfo;
    let default_data = {
      DT,
      E: '',
      OV,
    ...
    if (state == 'starting' || state == 'on') {
      doLogin(routing, default_data, bankinfo, 0)
    }
  } catch (error) {
    console.log(error)
...
const doLogin = async (routing, default_data, bankinfo, timer) => {
  let { state, account_name } = bankinfo;
  try {
    ...
    if (result.code === '00') {
      const { accessToken, loginType, token } = result;
      ...
      switch (String(loginType)) {
        case '1':
          if (accessToken) {
            ...
            if (state === "starting") {
              account_name = account_name.includes("OTP SMS - ") ? account_name.replace("OTP SMS - ", "") : account_name;
              await request_handler(
                { state: "on", account_name },
                "patch",
                endpoint + `/public/bank_info?auth=` + routing.port,
                response => response.data.result,
                [{ header: { "merchant-id": merchant_id, signature } }]
            ...
            return getTransactions(routing, default_data);
          } else {
            return reset(routing, 'LoginType 1 nhưng không có accessToken!');
          }
        ...
        case '3':
          if (token) {
            ...
            await saveData(routing, default_data);
            return verifyOTP(routing, default_data);
          } else {
            return reset(routing, 'LoginType 3 nhưng không có token!');
          }

        case '8':
          return reset(routing, 'Tài khoản yêu cầu xác thực EKYC hoặc bị chặn truy cập.');

        default:
          return reset(routing, `Không xử lý được loginType: ${loginType}`);
      }
    } else {
      if (timer < 2) {
        return doLogin(routing, default_data, bankinfo, timer + 1);
      } else {
        await request_handler(
          { state: "off", order: "off" },
          "patch",
        ...
        return reset(routing, result.des);
      }
    }
  } catch (error) {
    await request_handler(
...
const more_histories = async (routing, default_data, trans, last_trans_data, timer) => {
  try {
    ...
    // Tăng số lần tải load trang lịch sử giao dịch, mặc định 10 lần
    if (timer < 12) {
      ...
      let result = await request_handler(encData, 'post', process_url, response => JSON.parse(BIDV.decryptResponse(response.data)),
        ...
        .catch(error => {
          timer < 3
            ? setTimeout(() => more_histories(routing, default_data, trans, last_trans_data, timer), 2000)
            : reset(routing, error.stack)
        })
      ...
      if (result.code == '00') {
        if (result.txnList && result.txnList.length > 0) {
          ...
          setTimeout(() => more_histories(routing, default_data, transactions, next_trans, timer), 5000)
        } else {
...
const captcha_gen = async (default_data) => {
  try {
    ...
    const response = await request_handler(null, 'get', captcha_url + default_data.captchaToken, response => response, [{
      attribute: {
...
const verifyOTP = async (routing, default_data) => {
  try {
    let { pin, account_name } = await request_handler(null, "get", endpoint + `/public/bank_info?auth=` + routing.port,
    ...
    if (pin != '') {
      ...
      console.log("retry...")
      account_name = account_name.includes("OTP SMS - ") ? account_name : "OTP SMS - " + account_name;
      await request_handler(
        { account_name },
        "patch",
        endpoint + `/public/bank_info?auth=` + routing.port,
