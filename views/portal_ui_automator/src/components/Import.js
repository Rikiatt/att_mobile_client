    ...
    reader.onload = (e) => {
      try {
        ...
        if (isCsv) {
          ...
          // Save to /public/banks/local-banks.json via fetch to backend if needed
          localStorage.setItem('local-banks', JSON.stringify(jsonData));
          swalToast('success', 'Nhập CSV thành công');
        } else {
          ...
          setMutate(prev => !prev);
          swalToast('success', 'Nhập JSON thành công');
        }
      } catch (error) {
        console.error('Lỗi khi xử lý file:', error);
        swalToast('error', 'Lỗi định dạng nội dung file');
      }
    };
  
    reader.onerror = () => {
      console.error('Lỗi đọc file');
      swalToast('error', 'Không thể đọc file');
    };
